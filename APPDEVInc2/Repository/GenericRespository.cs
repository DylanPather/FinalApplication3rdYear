...
namespace APPDEVInc2.Repository
{ 
   ...
   public class GenericRespository<Tbl_Entity> : IRepository<Tbl_Entity> where Tbl_Entity : class
    {
        ...
        public int GetRecordCountWhere(Expression<Func<Tbl_Entity, bool>> wherePredict)
        {
            return _dbSet.Count(wherePredict);
        }
       ...
       // }
        public void UpdateProduct(StockTbl product , Tbl_Entity entity)
        {
            var existingproduct = _DBEntity.StockTbls.FirstOrDefault(p => p.StockID == product.StockID);

            if(existingproduct != null)
            {
                _dbSet.Attach(entity);
                _DBEntity.Entry(existingproduct).State = EntityState.Modified;
                _DBEntity.Entry(existingproduct).Property(x => x.Price).IsModified = false;
                _DBEntity.SaveChanges();
            }
